...
join_session (GstElement * pipeline, GstElement * rtpBin, SessionData * session)
{
  ...
  gst_bin_add_many (GST_BIN (pipeline), mprtpr, rtpSrc, rtcpSrc, rtcpSink, NULL);
  ...
  gst_element_link_pads (rtpSrc, "src", mprtpr, "mprtp_sink_1");
  gst_element_link_pads (mprtpr, "mprtp_src", rtpBin, padName);
