...
add_stream (GstPipeline * pipe, GstElement * rtpBin, SessionData * session)
{
  ...
  GstElement *rtpSink2 = gst_element_factory_make ("udpsink", NULL);
  ...
  gst_bin_add_many (GST_BIN (pipe), mprtps, rtpSink, rtcpSink, rtcpSrc, identity,
      session->input, NULL);
  ...
  g_object_set (rtpSink2, "port", basePort+1000, "host", "127.0.0.2", NULL);
...
//  g_free (padName);
  ...
  padName = g_strdup_printf ("send_rtp_src_%u", session->sessionNum);
  gst_element_link_pads (rtpBin, padName, mprtps, "mprtp_sink");
  gst_element_link_pads (mprtps, "mprtp_src_1", identity, "sink");
  gst_element_link (identity, rtpSink);
  gst_element_link_pads (mprtps, "mprtp_src_2", rtpSink2, "sink");
  g_free (padName);
